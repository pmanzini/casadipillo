...
function App() {
  ...
  return <>
    <header className="hero" id="home" style={{ backgroundImage: `linear-gradient(90deg, rgba(12, 35, 32, .82), rgba(12, 35, 32, .18)), url("${rootImage('monte.jpg', 'slideshow')}")` }}>
    ...
    <main>
      ...
      <GalleryStrip className="territory-gallery" kind="territory" images={territoryImages} label={t('territory.gallery')} onOpen={openGallery} />
    ...
    <footer id="contatti"><div className="container footer-grid"><div><a className="brand footer-brand" href="#home"><span className="brand-mark">⌂</span><span>Casa di Pillo<small>Frontino · Marche</small></span></a><p className="footer-note">Un luogo da abitare lentamente.</p></div><div><p className="eyebrow">{t('footer.contact')}</p><a href="mailto:pmanzini@hotmail.it">pmanzini@hotmail.it</a><a href="tel:+393358194110">+39 335 819 4110</a></div><div><p className="eyebrow">{t('footer.visit')}</p><a href="https://www.facebook.com/p/La-Casa-di-Pillo-a-Frontino-100042583646372/" target="_blank" rel="noreferrer">Facebook ↗</a></div></div><div className="container copyright">© {new Date().getFullYear()} B&B Casa di Pillo a Frontino. {t('footer.rights')}</div></footer>
    {gallery && <div className="lightbox" role="dialog" aria-modal="true" aria-label={gallery === 'house' ? t('home.gallery') : t('territory.gallery')} onClick={closeGallery}><button className="lightbox-close" aria-label={t('gallery.close')} onClick={closeGallery}>×</button><button className="lightbox-arrow left" aria-label={t('gallery.previous')} onClick={(event) => { event.stopPropagation(); move(-1); }}>‹</button><ImageWithLoader className="slideshow-image" src={image(galleryFolder, currentImages[activeIndex], 'slideshow')} alt="Casa di Pillo" onClick={(event) => event.stopPropagation()} /><button className="lightbox-arrow right" aria-label={t('gallery.next')} onClick={(event) => { event.stopPropagation(); move(1); }}>›</button></div>}
  </>;
}

function GalleryStrip({ className = '', kind, images, label, onOpen }: { className?: string; kind: GalleryKind; images: string[]; label: string; onOpen: (kind: GalleryKind, index: number) => void }) { return <div className={`gallery-wrap ${className}`.trim()} aria-label={label}><div className="gallery-track">{images.map((filename, index) => <button className="gallery-item" key={filename} onClick={() => onOpen(kind, index)} aria-label={`${label}: ${index + 1}`}><ImageWithLoader src={image(kind === 'house' ? 'lacasa' : 'ilterritorio', filename, 'carousel')} alt="" loading="lazy" /></button>)}</div></div>; }
function ImageWithLoader({ className = '', src, alt, loading, onClick }: { className?: string; src: string; alt: string; loading?: 'eager' | 'lazy'; onClick?: (event: MouseEvent<HTMLImageElement>) => void }) { const [status, setStatus] = useState<'loading' | 'loaded' | 'error'>('loading'); return <span className={`image-loader image-loader-${status} ${className}`.trim()} aria-busy={status === 'loading'}>{status === 'loading' && <span className="image-spinner" aria-label="Caricamento" />}<img src={src} alt={alt} loading={loading} onLoad={() => setStatus('loaded')} onError={() => setStatus('error')} onClick={onClick} /></span>; }
